...
#обработка данных
df = pd.read_csv("r6.2/logo.csv",nrows=10000,usecols=["user", "url", "activity"],)
#df.to_csv('r6.2/logo.csv', sep=',')
...
#приведение данных к виду для применения обработки
#converting to array
X=ds[:,1]
Y=ds[:,0]
#exit()
...
def lstm_model(array,test_array,num_words,num_of_cell):
    ...
    #log files
    with open('listfile-lstm-predict.txt', 'w') as filehandle:
        for listitem in predictedk:
            filehandle.write('%s\n' % listitem)
    with open('listfile-lstm-real.txt', 'w') as filehandle2:
...
def simplernn_model(array,test_array,num_words,num_of_cell):
    ...
    #log files
    with open('listfile-simplernn-predict.txt', 'w') as filehandle:
        for listitem in predictedk:
            filehandle.write('%s\n' % listitem)
    with open('listfile-simplernn-real.txt', 'w') as filehandle2:
        for listitem in Y_test:
            filehandle2.write('%s\n' % listitem)
...
def grumodel(array,test_array,num_words,num_of_cell):
    ...
    model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
    print(model.summary())
    model.fit(X_train,Y_train,epochs=1, nb_epoch=200,batch_size=128)
    predicted = model.predict(X_test)
    predictedk = model.predict( X_test)
    predicted = np.reshape(predicted, (predicted.size//nb_classes,nb_classes))
    predictedk = np.reshape(predictedk, (predictedk.size//nb_classes,nb_classes))
    scores = model.evaluate(X_test,Y_test, batch_size=32)
    print("Accuracy of GRU: %.2f%%" % (scores[1]*100))
    ...
    #log files
    with open('listfile-gru-predict.txt', 'w') as filehandle:
        for listitem in predictedk:
            filehandle.write('%s\n' % listitem)
    with open('listfile-gru-real.txt', 'w') as filehandle2:
        for listitem in Y_test:
            filehandle2.write('%s\n' % listitem)

train_size = int(len(encoded) * .75)
train, test = encoded[0:train_size], encoded[train_size:len(encoded)]
print("Test-1:LSTM 100 cells on 10000 elements")
lstm_model(train,test,num_words,100)
print("Test-2:GRU 100 cells on 10000 elements")
grumodel(train,test,num_words,100)
print("Test-3:SimpleRNN 100 cells on 10000 elements")
